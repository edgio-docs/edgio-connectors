...
describe('AngularRoutes', () => {
  ...
  describe('in local development', () => {
    it('should add routes for all static assets', async () => {
      ...
      expect(serveStatic).toHaveBeenCalledWith('dist/my-edgio-angular-app/browser/static_asset.txt')
    })
  ...
  describe('in the cloud', () => {
    ...
    it('should add a route for the service worker', async () => {
      ...
      await router.run(request, response)
      expect(serviceWorker).toHaveBeenCalledWith('dist/__edgio__/service-worker.js')
    })

    it('should add routes for all static assets', async () => {
      request.path = '/assets/static_asset.txt'
      await router.run(request, response)
      expect(serveStatic).toHaveBeenCalledWith('dist/my-edgio-angular-app/browser/assets/:path')
    })

    it('should add routes for all static chunks', async () => {
      request.path = '/chunk.js'
      await router.run(request, response)
      expect(serveStatic).toHaveBeenCalledWith('dist/my-edgio-angular-app/browser/chunk.js')
    })
  })
})
