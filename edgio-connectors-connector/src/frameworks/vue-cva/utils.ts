...
export async function getOutputDir(projectDir = process.cwd()) {
  ...
  if (projectType === PROJECT_TYPES.vite) {
    // This must be done through nonWebpackRequire, as the vite is already a dependency of target project, and we dont wish to bundle it
    const resolveConfig = nonWebpackRequire('vite').resolveConfig
    const config = (await resolveConfig({ mode: 'production' }, 'build', 'production')) ?? {}
    return relative(projectDir, config?.build?.outDir ?? 'dist')
  }

  // This must be done through nonWebpackRequire, as we mustnt bundle it
  const config = nonWebpackRequire(join(projectDir, configFile))
